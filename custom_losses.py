...
def dice_soft(y_true, y_pred, loss_type='sorensen',
              axis=[1, 2, 3], smooth=1e-5, from_logits=False):
    ...
    smooth : float
        ...
        If either y_pred or y_true are empty (all pixels are background),
        dice = ```smooth/(small_value + smooth)``,
        then if smooth is very small, dice close to 0
        (even the image values lower than the threshold),
    ...
    if loss_type == 'jaccard':
        ll = tf.reduce_sum(y_pred * y_pred, axis=axis)
        r = tf.reduce_sum(y_true * y_true, axis=axis)
    elif loss_type == 'sorensen':
        ll = tf.reduce_sum(y_pred, axis=axis)
    ...
    # new haodong
    dice = (2. * inse + smooth) / (ll + r + smooth)
    ##
...
def dice_hard(y_true, y_pred, threshold=0.5, axis=[1, 2, 3], smooth=1e-5):
    ...
    # new haodong
    hard_dice = (2. * inse + smooth) / (ll + r + smooth)
    ##
...
def margin_loss(margin=0.4, downweight=0.5, pos_weight=1.0):
    ...
    def _margin_loss(labels, raw_logits):
        ...
        Each wrong logit costs its distance to margin.
        For negative logits margin is
        0.1 and for positives it is 0.9. First subtract 0.5 from all logits.
