...
class ConvCapsuleLayer(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.b = self.add_weight(shape=[1, 1, self.num_capsule,
                                 self.num_atoms],
    ...
    def call(self, input_tensor, training=None):
        ...
        votes = K.reshape(conv, [input_shape[1], input_shape[0],
                          votes_shape[1], votes_shape[2],
                          self.num_capsule, self.num_atoms])
    ...
    def get_config(self):
        temp_ini = self.kernel_initializer
        config = {
            ...
            'kernel_initializer': initializers.serialize(temp_ini)
        }
...
class DeconvCapsuleLayer(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self.upsamp_type == 'subpix':
            temp_num = self.num_capsule * self.num_atoms *\
                self.scaling * self.scaling
            self.W = self.add_weight(shape=[self.kernel_size, self.kernel_size,
                                     self.input_num_atoms, temp_num],
                                     initializer=self.kernel_initializer,
                                     name='W')
        elif self.upsamp_type == 'resize':
            temp_num = self.input_num_atoms, self.num_capsule * self.num_atoms
            self.W = self.add_weight(shape=[self.kernel_size, self.kernel_size,
                                     temp_num],
                                     initializer=self.kernel_initializer,
                                     name='W')
    ...
    def call(self, input_tensor, training=None):
        ...
        votes = K.reshape(outputs, [input_shape[1], input_shape[0],
                          votes_shape[1], votes_shape[2],
                          self.num_capsule, self.num_atoms])
        votes.set_shape((None, self.input_num_capsule, conv_height.value,
                        conv_width.value, self.num_capsule, self.num_atoms))
        ...
        logit_shape = K.stack([input_shape[1], input_shape[0], votes_shape[1],
                              votes_shape[2], self.num_capsule])
        biases_replicated = K.tile(self.b, [votes_shape[1],
                                   votes_shape[2], 1, 1])
