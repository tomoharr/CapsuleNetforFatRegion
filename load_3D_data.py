...
def load_data(root, split):
    ...
    # ./data_root_dir/split_lists/train_split_n.csv
    with open(join(root,
              'split_lists', 'train_split_' + str(split) + '.csv'), 'r') as f:
        reader = csv.reader(f)
...
def compute_class_weights(root, train_data_list):
    '''
        We want to weight the the positive pixels
        by the ratio of negative to positive.
        Three scenarios:
            1. Equal classes. neg/pos ~ 1. Standard binary cross-entropy
            2. Many more negative examples.
            The network will learn to always output negative.
            In this way we want to increase the punishment for getting
            a positive wrong that way it will want to put positive more
            3. Many more positive examples.
            We weight the positive value less so that negatives have a chance.
    '''
    pos = 0.0
    neg = 0.0
    for img_name in tqdm(train_data_list):
        img = sitk.GetArrayFromImage(sitk.ReadImage(
                                     join(root, 'masks', img_name[0])))
        for slic in img:
            if not np.any(slic):
                continue
            else:
...
def load_class_weights(root, split):
    temp_name = 'train_split_' + str(split) + '_class_weights.npy'
    class_weight_filename = join(root, 'split_lists', temp_name)
    try:
        return np.load(class_weight_filename)
    except FileNotFoundError:
        print('Class weight file {} not found.\nComputing class weights now.'
              ' This may take some time.'.format(class_weight_filename))
        ...
        print('Finished computing class weights.'
              'This value has been saved for this training split.')
        return value
...
def split_data(root_path, num_splits=4):
    ...
    assert len(mask_list) != 0,\
        'Unable to find any files in {}'.format(join(root_path, 'masks'))
    ...
    for train_index, test_index in kf.split(mask_list):
        with open(join(outdir, 'train_split_' + str(n) + '.csv'),
                  'wb') as csvfile:
        ...
        with open(join(outdir,
                  'test_split_' + str(n) + '.csv'), 'wb') as csvfile:
            writer = csv.writer(csvfile, delimiter=',', quotechar='|',
                                quoting=csv.QUOTE_MINIMAL)
            for i in test_index:
                writer.writerow([basename(mask_list[i])])
...
def convert_data_to_numpy(root_path, img_name, no_masks=False,
                          overwrite=False):
    fname = img_name[:-4]
    ...
    try:
        ...
        if not no_masks:
            ...
            # Class 3 intendity:127
            # Class 4 intensity:191
            # Other Class 0
            mask[mask == 0] = 1
            mask[mask != 1] = 0
            # =====================================================
        ...
        try:
            ...
            ax[0].set_title('Slice {}/{}'.format(
                            img.shape[2] // 3, img.shape[2]))
            ax[0].axis('off')

            ax[1].imshow(img[:, :, img.shape[2] // 2], cmap='gray')
            if not no_masks:
                ax[1].imshow(mask[:, :, img.shape[2] // 2], alpha=0.15)
            ax[1].set_title('Slice {}/{}'.format(
                            img.shape[2] // 2, img.shape[2]))
            ax[1].axis('off')

            ax[2].imshow(img[:, :, img.shape[2] // 2 + img.shape[2] // 4],
                         cmap='gray')
            if not no_masks:
                ax[2].imshow(mask[:, :, img.shape[2] // 2 + img.shape[2] // 4],
                             alpha=0.15)
            ax[2].set_title('Slice {}/{}'.format(
                            img.shape[2] // 2 + img.shape[2] // 4,
                            img.shape[2]))
            ax[2].axis('off')
            ...
            plt.savefig(join(fig_path, fname + '.png'),
...
def augmentImages(batch_of_images, batch_of_masks):
    for i in range(len(batch_of_images)):
        img_and_mask = np.concatenate((batch_of_images[i, ...],
                                      batch_of_masks[i, ...]), axis=2)
        if img_and_mask.ndim == 4:
        ...
        if np.random.randint(0, 5) == 3:
            img_and_mask = elastic_transform(img_and_mask, alpha=1000,
                                             sigma=80, alpha_affine=50)

        if np.random.randint(0, 10) == 7:
            img_and_mask = random_shift(img_and_mask, wrg=0.2, hrg=0.2,
                                        row_axis=0, col_axis=1, channel_axis=2,
                                        fill_mode='constant', cval=0.)

        if np.random.randint(0, 10) == 7:
            img_and_mask = random_shear(img_and_mask, intensity=16, row_axis=0,
                                        col_axis=1, channel_axis=2,
                                        fill_mode='constant', cval=0.)

        if np.random.randint(0, 10) == 7:
            img_and_mask = random_zoom(img_and_mask, zoom_range=(0.75, 0.75),
                                       row_axis=0, col_axis=1, channel_axis=2,
                                       fill_mode='constant', cval=0.)
        ...
        if batch_of_images.ndim == 4:
            batch_of_images[i, ...] =\
                img_and_mask[..., 0:img_and_mask.shape[2]//2]
            batch_of_masks[i, ...] =\
                img_and_mask[..., img_and_mask.shape[2]//2:]
...
def generate_train_batches(root_path, train_list, net_input_shape,
                           net, batchSize=1, numSlices=1, subSampAmt=-1,
                           stride=1, downSampAmt=1, shuff=1, aug_data=1):
    # Create placeholders for training
    img_batch = np.zeros((np.concatenate(((batchSize,),
                         net_input_shape))), dtype=np.float32)
    mask_batch = np.zeros((np.concatenate(((batchSize,),
                          net_input_shape))), dtype=np.uint8)
        ...
        for i, scan_name in enumerate(train_list):
            try:
                ...
                train_img, train_mask =\
                    convert_data_to_numpy(root_path, scan_name)
            ...
            for j in indicies:
                ...
                if img_batch.ndim == 4:
                    img_batch[count, :, :, :] =\
                        train_img[:, :, j:j + numSlices * (subSampAmt+1):subSampAmt+1]
                    mask_batch[count, :, :, :] =\
                        train_mask[:, :, j:j + numSlices * (subSampAmt+1):subSampAmt+1]
                elif img_batch.ndim == 5:
                    # Assumes img and mask are single channel.
                    # Replace 0 with : if multi-channel.
                    img_batch[count, :, :, :, 0] =\
                        train_img[:, :, j:j + numSlices * (subSampAmt+1):subSampAmt+1]
                    mask_batch[count, :, :, :, 0] =\
                        train_mask[:, :, j:j + numSlices * (subSampAmt+1):subSampAmt+1]
                ...
                if count % batchSize == 0:
                    count = 0
                    if aug_data:
                        img_batch, mask_batch = augmentImages(img_batch,
                                                              mask_batch)
                    if debug:
                        if img_batch.ndim == 4:
                        ...
                        plt.close()
                    if net.find('caps') != -1:
                        yield ([img_batch, mask_batch], [mask_batch,
                               mask_batch*img_batch])
                    else:
...
@threadsafe_generator
def generate_val_batches(root_path, val_list, net_input_shape, net,
                         batchSize=1, numSlices=1, subSampAmt=-1,
    ...
    while True:
        ...
        for i, scan_name in enumerate(val_list):
            ...
            indicies =\
                np.arange(0, val_img.shape[2] -numSlices * (subSampAmt + 1) + 1, stride)
            if shuff:
                shuffle(indicies)
            ...
            for j in indicies:
                ...
                if img_batch.ndim == 4:
                    img_batch[count, :, :, :] =\
                        val_img[:, :, j:j + numSlices * (subSampAmt+1):subSampAmt+1]
                    mask_batch[count, :, :, :] =\
                        val_mask[:, :, j:j + numSlices * (subSampAmt+1):subSampAmt+1]
                elif img_batch.ndim == 5:
                    # Assumes img and mask are single channel.
                    # Replace 0 with : if multi-channel.
                    img_batch[count, :, :, :, 0] =\
...
@threadsafe_generator
def generate_test_batches(root_path, test_list, net_input_shape, batchSize=1,
                          numSlices=1, subSampAmt=0,
    ...
    for i, scan_name in enumerate(test_list):
        ...
        for j in indicies:
            if img_batch.ndim == 4:
                img_batch[count, :, :, :] =\
                    test_img[:, :, j:j + numSlices * (subSampAmt+1):subSampAmt+1]
            elif img_batch.ndim == 5:
                # Assumes img and mask are single channel.
                # Replace 0 with : if multi-channel.
                img_batch[count, :, :, :, 0] =\
