...
# =====================================================================
# tiramisu
def DenseNetFCN(input_shape, nb_dense_block=5,
                growth_rate=16, nb_layers_per_block=4,
    ...
    if upsampling_type not in ['upsampling', 'deconv', 'subpixel']:
        raise ValueError('Parameter "upsampling_type" must be '
                         'one of "upsampling", '
                         '"deconv" or "subpixel".')
    ...
    if type(nb_layers_per_block) is not list and nb_dense_block < 1:
        raise ValueError('Number of dense layers per block '
                         'must be greater than 1. Argument '
    ...
    x = __create_fcn_dense_net(classes, img_input, include_top, nb_dense_block,
                               ...
                               nb_layers_per_block, upsampling_conv,
                               upsampling_type,
...
def __dense_block(x, nb_layers, nb_filter, growth_rate,
...
def __transition_up_block(ip, nb_filters, type='deconv',
                          weight_decay=1E-4, block_prefix=None):
    ...
    with K.name_scope('TransitionUp'):
        ...
        if type == 'upsampling':
            x = UpSampling2D(name=name_or_none(block_prefix,
                                               '_upsampling'))(ip)
        elif type == 'subpixel':
            ...
            x = Conv2DTranspose(nb_filters, (3, 3), activation='relu',
    ...
    with K.name_scope('DenseNetFCN'):
        ...
        if reduction != 0.0:
            if not (reduction <= 1.0 and reduction > 0.0):
                raise ValueError('`reduction` value must lie '
                                 'between 0.0 and 1.0')

        # check if upsampling_conv has minimum number of filters
        # minimum is set to 12, as at least 3 color channels
        # are needed for correct upsampling
        if not (nb_upsampling_conv > 12 and nb_upsampling_conv % 4 == 0):
            raise ValueError('Parameter `nb_upsampling_conv` '
                             'number of channels must '
                             'be a positive number divisible '
                             'by 4 and greater than 12')
        ...
        if type(nb_layers_per_block) is list or type(nb_layers_per_block) is tuple:
            ...
            if len(nb_layers) != (nb_dense_block + 1):
                raise ValueError('If `nb_dense_block` is a list, '
                                 'its length must be '
                                 '(`nb_dense_block` + 1)')
        ...
        # return the concatenated feature maps
        # without the concatenation of the input
        _, nb_filter, concat_list =\
            __dense_block(x, bottleneck_nb_layers, nb_filter, growth_rate,
        ...
        for block_idx in range(nb_dense_block):
            ...
            # upsampling block must upsample only the feature maps
            # (concat_list[1:]),
            ...
            x_up, nb_filter, concat_list =\
                __dense_block(x, nb_layers[nb_dense_block + block_idx + 1],
